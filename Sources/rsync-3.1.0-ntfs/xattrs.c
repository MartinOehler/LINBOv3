 ...
 * but we need them in order to clone NTFS.
 * reparse_data is a special case (symlink), apparently it cannot set directly via
 * setxattr, so we need to handle this outside of rsync in order to avoid protocol breakage.
