...
#define PREALLOC 1048576 /* Overcommit 1MB for extensions of hive */
...
int find_free_blk(struct hive *hdesc, int pofs, int size)
{
...
#if 0
  printf("find_free_blk() checking page 0x%x, size %d, page len 0x%x\n",
           pofs, size, p->ofs_next);
#endif
  ...
  while (vofs-pofs < (p->ofs_next - HBIN_ENDFILL)) {
    ...
    if (seglen == 0) {
      ...
      printf("vofs: %d, pofs: %d, vofs-pofs: %d, p->ofs_next=%ld\n",vofs,pofs,vofs-pofs,p->ofs_next);
      debugit(hdesc->buffer,hdesc->size);
      return(0);
    }
...
int find_free(struct hive *hdesc, int size)
{
  ...
  r = 0x1000;
  printf("Looking for free block in hive, hdesc->size = %d, hdesc->pages = %d pages\n", hdesc->size, hdesc->pages);
  while (r < hdesc->size) {
    h = (struct hbin_page *)(hdesc->buffer + r);
    if (h->id != 0x6E696268) return(0);
    if (h->ofs_next == 0) {
...
unsigned long hive_checksum(struct hive *hdesc)
{
 unsigned long *hdw = (unsigned long *)hdesc->buffer;
 unsigned long checksum;
 int i;

 /* sum up all dwords from 0x0 to 0x000001FB,
  * see ntreg.h regf_header->checksum
  */
#if 0
 printf("    hdw[0]=%ld /* id */\n       + hdw[1]=%ld /* unknown1 */\n    + hdw[2]=%ld /* unknown2 */\n"
        "  + hdw[5]=%ld /* unknown3\n */ + hdw[6]=%ld /* unknown4 */\n    + hdw[7]=%ld /* unknown5 */"
	"  + hdw[8]=%ld /* unknown6 */\n + hdw[9]=%ld /* ofs_rootkey */\n + hdw[10]=%ld /* filesize */\n"
	"  + hdw[11]=%ld /* unknown7 */\n",
	hdw[0], hdw[1], hdw[2], hdw[5], hdw[6], hdw[7], hdw[8], hdw[9], hdw[10], hdw[11]);
 return    hdr->id + hdr->unknown1 + hdr->unknown2 + hdr->unknown3
            + hdr->unknown4 + hdr->unknown5 + hdr->unknown6
	    + hdr->ofs_rootkey + hdr->filesize + hdr->unknown7;
#endif
	/* Looks like we have to use an XOR checksum here. -KK */
	for(checksum=0,i=0x0000; i<0x001FC/sizeof(long); i++) {
		checksum ^= hdw[i];
	}
	return checksum;
}
...
/* extend hive by a page of <size> bytes (including header!)
 * and mark data section of page as empty block.
 * It seems that "size" should be 0x1000.
 * Returns allocated block offset if successful, 0 if error. */
int extend_hive(struct hive *hdesc, int size) {
 static int freespace = PREALLOC;
 int oldsize = hdesc->size;
 unsigned long oldsum = hive_checksum(hdesc);
 int r;
 char *garbage = NULL;
 struct hbin_page *newpage, *lastpage = NULL;
 struct regf_header *hdr;

 if(size > freespace) {
   printf("extend_hive: Out of preallocated memory for extension, please increase PREALLOC definition.\n");
   return 0;
 }

 if (hdesc->state & HMODE_NOALLOC)
  {
   printf("extend_hive: ERROR: Hive %s is in no allocation safe mode,"
          "new space not allocated. Operation will fail!\n", hdesc->filename);
   return 0 ;
  }

 /* find last valid page in file. */
 for(r = 0x1000; r < oldsize; r += lastpage->ofs_next) {
  struct hbin_page *p = (struct hbin_page *)(hdesc->buffer + r);
  if (p->id != 0x6E696268 || p->ofs_next == 0 ) {
   garbage = (char *)p;
   printf("WARNING: Adding page before garbage at %p.\n", garbage);
   printf("         ofs_next = %ld.\n", p->ofs_next);
   printf("         ofs_from1 = %ld.\n", p->ofs_from1);
   break; /* invalid next page */
  }
  lastpage = p;
 }

 if(!lastpage) {
   printf("extend_hive: lastpage=NULL, something is seriously wrong here!\n");
   return 0; /* leave everything untouched. */
 }

 freespace -= size;
 newpage = (struct hbin_page *)((char *)lastpage + lastpage->ofs_next);
 printf("New page added at %p.\n", newpage);

 if(garbage) /* Copy old garbage to end after page */
  {
   printf("Copying garbage from %p to %p, size %d\n", garbage, newpage + size, (hdesc->buffer - (char *)garbage) + oldsize);
   memmove((char *)newpage + size, garbage, (hdesc->buffer - (char *)garbage) + oldsize);
   // ((struct hbin_page *)((char *)newpage+size))->ofs_from1 = (char *)newpage + size - hdesc->buffer - 0x1000;
  }

 /* initialize new page */
 memset(newpage, 0 , size);
 newpage->id = 0x6E696268; /* hbin signature */
 newpage->ofs_from1 = (char *)newpage - hdesc->buffer - 0x1000;
 newpage->ofs_next = size; /* Relative offset to next hbin. */
 newpage->len_page = size - 0x0020; /* Meaning is unknown yet, probably data size in bytes. */
 hdesc->state |= HMODE_DIRTY;
 hdesc->size  += size;
 hdesc->pages += 1;
 hdesc->unuseblk += 1;
 hdesc->unusetot += (size - 0x20 - 4); /* without page and block header */
 hdesc->usetot += (0x20 + 4);          /* page and block header in use */
 hdr = (struct regf_header *)hdesc->buffer;
 printf("increasing filesize from %ld to %ld.\n", hdr->filesize, hdr->filesize + size);
 hdr->filesize += size;
 printf("checksum: old %ld (calculated %ld)", hdr->checksum, oldsum);
 hdr->checksum = (long) hive_checksum(hdesc);
 printf(", new %ld\n", hdr->checksum);
 /* now, link this page to the previous one in the hive. */
 lastpage->ofs_next = (char *)newpage - (char *)lastpage;
 printf("inserting int.\n");
 /* Create a new (unused) block on the page. */
 *((int *)(&newpage->data[0])) = (size - 0x20 - 4); /* new block size, positive for unused */
 printf("Returning new page.\n");
 return &newpage->data[0] - hdesc->buffer; /* return relative block position */
}
...
int alloc_block(struct hive *hdesc, int ofs, int size)
{
  ...
  if(!blk) {
    printf("alloc_block warning: No contiguous space "
           "for block of %d bytes, extending hive.\n", size);
    blk = extend_hive(hdesc, 0x1000);
 }
  ...
  if (blk) {  /* Got the space */
    ...
    *(int *)((hdesc->buffer)+blk) = -(size);
    ...
    if (trailsize) {
      ...
      printf("alloc_block: added new trailing block of %d bytes.\n", trailsize);
...
int vlist_find(struct hive *hdesc, int vlistofs, int numval, char *name)
{
  ...
  for (i = 0; i < numval; i++) {
    ...
    if (!strncasecmp(name, vkkey->keyname, strlen(name))) { /* name match? */
      return(i);
    }
  }
  return(-1);
  ...
  if (key->no_subkeys > 0) {    /* If it has subkeys, loop through the hash */
	...
	if (newnkkey->id != 0x6b6e) {
	  ...
	  if (newnkkey->len_name <= 0) {
	    ...
	    if (!strncasecmp(part,newnkkey->keyname,plen)) {
...
int del_key(struct hive *hdesc, int nkofs, char *name)
{
  ...
  do {   /* 'ri' loop, at least run once if no 'ri' deep index */
	...
	onkofs = oldli->hash[o].ofs_nk;
	onk = (struct nk_key *)(onkofs + hdesc->buffer + 0x1004);
	if (slot == -1 && onk->len_name == namlen && !strncasecmp(name, onk->keyname, (onk->len_name > namlen) ? onk->len_name : namlen)) {
	  slot = o;
	...
	onkofs = oldlf->hash[o].ofs_nk;
	onk = (struct nk_key *)(onkofs + hdesc->buffer + 0x1004);
	if (slot == -1 && (onk->len_name == namlen) && !strncasecmp(name, onk->keyname, onk->len_name)) {
	  slot = o;
	  delnkofs = onkofs; delnk = onk;
	  rimax = rislot;
	  o++;
...
struct hive *openHive(char *filename, int mode)
{
  ...
  ALLOC(hdesc->buffer,1,hdesc->size + PREALLOC); /* Leave some space for extensions */
  ...
  /* NOTE/KLUDGE: Assume first page starts at offset 0x1000 */
   ...
   printf("Hive header checksum 0x%lx, calculated 0x%lx\n",((struct regf_header *)hdesc->buffer)->checksum, hive_checksum(hdesc));
   printf("Hive header filesize %ld\n",((struct regf_header *)hdesc->buffer)->filesize);
